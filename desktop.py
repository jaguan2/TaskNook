...
if getattr(sys, "frozen", False):
    ...
    # and the SQLite DB must go somewhere writable (not the read-only bundle).
    BASE_DIR = sys._MEIPASS
    data_home = os.environ.get("LOCALAPPDATA") or os.path.expanduser("~")
    db_dir = os.path.join(data_home, "TaskNook")
    os.makedirs(db_dir, exist_ok=True)
    os.environ.setdefault("TASKNOOK_DB", os.path.join(db_dir, "tasknook.db"))
else:
...
def main():
    ...
    try:
        ...
        webview.start()  # blocks until the window is closed; daemon server exits
